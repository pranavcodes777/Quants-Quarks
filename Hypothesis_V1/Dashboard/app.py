...
<style>
  ...
  /* ── Tabs — bordered, distinctive ── */
  div[data-testid="stTabs"] button {
    font-size: 0.78rem; letter-spacing: 0.06em;
    border: 1px solid rgba(128,128,128,0.22) !important;
    border-bottom: none !important;
    border-radius: 7px 7px 0 0 !important;
    margin: 0 2px !important; padding: 6px 18px !important;
  ...
  /* ── Expanders — full-bar collapsible sections ── */
...
<style>
  ...
  /* ── Tabs ── */
  div[data-baseweb="tab-list"]          { background: #e8edf2 !important; border-radius: 8px 8px 0 0 !important;
                                          padding: 3px 3px 0 !important; }
  div[data-testid="stTabs"] button      { border-color: #cbd5e1 !important;
                                          background: #f1f5f9 !important; color: #64748b !important; }
  div[data-testid="stTabs"] button:hover { border-color: #16a34a !important; color: #16a34a !important; }
  div[data-testid="stTabs"] button[aria-selected="true"]
                                        { border-color: #16a34a !important;
                                          background: #ffffff !important; color: #15803d !important;
...
with tab2:
    ...
    with st.expander(f"Trend over Time — {meta['label']}", expanded=True):
        fig_line = px.line(
    ...
    with st.expander(f"Distribution by Company — {meta['label']}", expanded=True):
        fig_box = px.box(
    ...
    with st.expander("Summary Statistics by Company", expanded=False):
        stats_tbl = (
...
with tab3:
    ...
    with st.expander(f"Peer Percentile Ranking — {snap_co}  ·  FY{snap_yr}", expanded=True):
    ...
    with st.expander("Factor Detail — Values & Peer Rank", expanded=True):
    ...
    with st.expander("Historical Factor Data — All Years", expanded=False):
...
with tab4:
    ...
    with st.expander(f"Factor IC Rankings — {len(ic_filt)} factors pass filters  ·  {len(ic_all)} analysed", expanded=True):
        # ── KPI strip ─────────────────────────────────────────────────────────
    ...
    with st.expander("Factor Deep Dive — Year-by-Year IC Chart & Interpretation", expanded=True):
...
with tab5:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    with st.expander("Section 1 — Group Composite IC Rankings  ·  all groups scored & compared", expanded=True):
    ...
    with st.expander("R² and Beta — Variance Explained & Economic Size by Group", expanded=False):
    ...
    # ══════════════════════════════════════════════════════════════════════════
    with st.expander("Section 2 — Style Composite Builder  ·  combine groups into one style score", expanded=True):
        ...
        if len(sel_style) < 2:
            ...
            style_beta_by_yr:   dict[int, float] = {}
            ...
            for yr in all_yrs:
                ...
                try:
                    ...
                    pass
                try:
                    _sv = aligned["style_score"].values.astype(float)
                    _rv = aligned["Return_1Y_Fwd"].values.astype(float)
                    _lo, _hi = float(np.percentile(_sv, 5)), float(np.percentile(_sv, 95))
                    _sv_w = np.clip(_sv, _lo, _hi) if _lo < _hi else _sv
                    _slope = float(np.polyfit(_sv_w, _rv, 1)[0])
                    if not np.isnan(_slope):
                        style_beta_by_yr[yr] = _slope
                except Exception:
                    pass
            ...
            if len(style_ic_by_yr) < 2:
                ...
                s_r2      = round(float(np.mean([v ** 2 for v in sic_vals])) * 100, 2)
                s_beta    = round(float(np.mean(list(style_beta_by_yr.values()))), 4) if style_beta_by_yr else float("nan")
                try:
                ...
                sk6.metric("R² (%)",         f"{s_r2:.1f}%",
                           help="Mean IC² × 100 — average variance in returns explained per year")
                sk7.metric("Beta",           f"{s_beta:+.2f}" if not np.isnan(s_beta) else "—",
                           help="OLS slope: avg % return shift per 1-unit increase in style score (winsorized 5–95%)")
...
with tab6:
    ...
    # ══════════════════════════════════════════════════════════════════════════
    with st.expander("Chart 1 — Average Annual Return by Quintile", expanded=True):
        ...
        # ══════════════════════════════════════════════════════════════════════════
    with st.expander("Chart 2 — Q1 − Q5 Spread by Year  ·  consistency check", expanded=True):
        ...
        # ══════════════════════════════════════════════════════════════════════════
    with st.expander("Chart 3 — Cumulative Wealth  ·  ₹100 at start", expanded=True):
